...
## Submission envelope (non-normative)

When a record is POSTed somewhere, the body is the record itself, optionally
with one extra top-level field:

| field | type | meaning |
|---|---|---|
| `contact` | string | An address the submitter chose to attach. Strictly opt-in: the key is **absent** — not empty, not null — unless the submitter explicitly supplied it, and it is never written into an exported file. |

Producers should print the exact bytes before sending them and should offer a
dry run that prints without sending, so a user can see precisely what would
leave the machine. Receivers should treat `contact` as personal data and
everything else as publishable.
