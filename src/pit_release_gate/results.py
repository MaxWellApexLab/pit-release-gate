...
so nothing that builds or writes a record can send it anywhere. Submission
lives in :mod:`pit_release_gate.submit` and runs only when the user asks for
...
def validate_results(obj) -> list[str]:
    ...
    check a record before trusting it, and so that :mod:`pit_release_gate.submit`
    can refuse to send something malformed.
    """
    problems: list[str] = []
...
def dumps_results(obj) -> str:
    """Canonical JSON text of a record (what --export writes, what --submit sends)."""
